...
class PiMetroSupabaseUser extends BaseAuthUser {
  PiMetroSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> piMetroSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PiMetroSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
